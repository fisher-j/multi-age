...
d$transects |> select(!any_of(transectid))
veg <- d$transects |>
  ...
  mutate(
    ...
    woody = ((live_woody + dead_woody) / 100) * avg_w_ht * 18,
    woody_prop_dead = if_else(woody == 0, 0, dead_woody / (live_woody + dead_woody)),
    herb = ((live_herb + dead_herb) / 100) * avg_h_ht * 8,
    herb_prop_dead = if_else(herb == 0, 0, dead_herb / (live_herb + dead_herb))
  ) |>
  select(!matches("live_|dead_"), woody_ht = avg_w_ht, herb_ht = avg_h_ht) 
```
