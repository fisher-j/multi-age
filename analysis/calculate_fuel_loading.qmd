...
if (setequal(names(cwd), names(fwd))) woody_fuel <- bind_rows(fwd, cwd)
cwd <- woody_fuel |> 
...
tribble(
  ...
  "Annadel SP & Humboldt Redwoods SP (rw dbh <= 60 in)", 7.15,

  "@kittredgeComparisonForestFloors1940",
  "plantation redwoods", 6.8,

  "Stuart, J.D. 1985, Unpubl. in @finneyFuelLoadingBulk1993",
  "Redwoods SP, (mean of duff & litter)", 9.25,

  "@vanwagtendonkFuelBedCharacteristics1998",
  "Avg. for Sierra Nevada conifers", 16.24
  
) |> knitr::kable()
```

On average, we have 50/50 duff/litter, using the average of the above redwood
forest types (7.73 Mg ha^-1^ cm^-1^)

```{r}
summarize(
  dufflitter,
  pct_litter = mean(pct_litter, na.rm = TRUE),
  total_depth = mean(duff_litter)
)
```
