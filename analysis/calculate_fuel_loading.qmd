...
cwd <- d$coarse_woody |>
  ...
  select(all_of(transectid), slope, class, sum_d2, length = "thoushr_length")
...
We will use the sum of squared diameters as the `count` column, which just gets
multiplied by 1 -- the corresponding `d2` parameter.

```{r}

woodydebris <- bind_rows(fwd, rename(cwd, count = sum_d2))
