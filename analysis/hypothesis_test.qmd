...
positive support only, and more approriate priors would improve the model.
...
data. They are constrained to be positive, as it's not possible to have a
negative fuel load.

```{r define-priors}
#| label: fig-priors
#| fig-cap: Normal priors for the intercept are centered at the median and sd of 2.5 times the sd of
#|   the data. For the fixed effect treatmen, they are centered at zero with the
#|   a standard deviation equal to 1 sd of the data.
...
d <- mutate(d, 
  priors = list(
    ...
    brms::set_prior(
      str_glue(
        "normal( 0, {round(2.5 * sd(data$load))} )"
      ), 
      class = "b"
    )
  )
...
d |>
  mutate(
    prior_dist = list(tidybayes::parse_dist(priors) |> rename(dist_class = class))
  ) |>
  unnest(prior_dist) |>
  ggplot(aes(ydist = .dist_obj, color = coef)) +
    ...
    geom_text(
      aes(label = format(.dist_obj), y = 0, x = 0.97, vjust = 0),
      position = position_dodge(width = 0.3)
    ) +
    coord_flip() +
    facet_wrap(~ class, scales = "free_x") +
    scale_color_hue(labels = c("treatment", "intercept"))
