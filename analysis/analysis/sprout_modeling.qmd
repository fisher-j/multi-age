...
# nolint iwalk(minc, label_model_formula)
```
...
minc14 = glmmTMB(
  ht_inc ~ treat + year * spp + (1 | tree) + (1 | plot) + (1 | plot:spp),
  dispformula = ~ spp * year * treat,
  data = dinc
)
...
minc16 <- glmmTMB(
  ht_inc ~ treat * spp * year + (1 | tree) + (0 + spp | plot),
  dispformula = ~ spp * year * treat,
  data = dinc
)

modelsummary(
  list(minc[[12]], minc13, minc14, minc15, minc16),
...
# # doesn't. I also check the three-way interaction again.
# modelsummary(
#   list(mht109, mht1010, mht1011),
#   stars = TRUE,
#   output = "markdown",
...
plot(simulateResiduals(mht1011))
```
