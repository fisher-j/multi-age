...
for dispersion. Here it is displayed with the conditional estimates only.
...
emm_treat <- emmeans(minc[[12]], "treat", by = "spp")
...
Redwood grew faster than tanoak, but slowed down more relative
to it in the second period. Height increment for redwood was 0.42 m yr^-1^ greater in
the first period and 0.33 m yr^-1^ greater in the second period than tanoak
height increment.
...
emmeans(minc[[12]], c("year"), by = "spp") |> plot(comparisons = TRUE)
```
...
minc11em <- emmeans(minc[[12]], "year", by = c("spp"))
...
#| fig-cap: >
#|   Predicted mean height and 95% confidence intervals for redwood and tanoak
#|   stump sprouts 10 years after harvest using four different harvest
#|   treatments. The letters indicate statistically significant differences
#|   between treatments within a species.
