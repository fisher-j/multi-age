...
The first two models don't include the site as strata. When I put site in the
model as a fixed effect, p-values increase slightly.
...
If I put site in `strata`, then it doesn't seem to matter if I include it as a
fixed effect or not.
...
If I try to do `strata` with the interaction of site and treatment, then the
model seems to not work. Perhaps this is because there is only one of each
site:treatment interaction.

```{r}

with(ba_ha, adonis2(
  cbind(rw, to, df, other) ~ treat,
  permutations = perms,
  strata = interaction(site, treat)
))
...
I'm not sure what it means to include site as a main effect, interaction, AND as
the strata
