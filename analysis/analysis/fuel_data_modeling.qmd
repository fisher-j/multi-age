...
fuel_class_labels2 <- imap(fuel_class_labels, \(x, i)  eval(bquote(`~`(.(i), .(x)))))
...
This package also allows fitting flexible models, but unlike the Bayesian
approach, it is fast, and familiar as it works in the frequentist paradigm.

```{r}
#| echo: false

# helper function for selecting a model from a list based on class
...
run_glmmtmb_mod <- function(d, mods) {
  ...
  mutate(
    ...
    .emmeans = list(
      emmeans::emmeans(mod, "treatment", type = "response", re.form = NA)
    ),
    emmeans = list(as_tibble(.emmeans)),
    pairs = list(as_tibble(pairs(.emmeans, infer = TRUE))),
    aic = AIC(mod),
...
# The pre-pct data
dpre <- load2("long", "pre", -c(phase, azi)) |> nest(.by = class) |> rowwise()
...
mods <- alist(
  ...
  default = glmmTMB(form, data, family = ziGamma(link = "log"), 
    ziformula = ~1
  ),
  tenhr = glmmTMB(form, data, family = ziGamma(link = "log"),
    ziformula = ~ treatment,
...
fuel_tmb$pre |>
  ...
  plot(x$.emmeans[[1]], comparisons = TRUE) +
...
form <- load ~ treatment + (1 | site/treatment/corner)
mods <- alist(
  default = glmmTMB(form, data,
    family = ziGamma(link = "log"),
    ziformula = ~ 1
  ),
  onehr = glmmTMB(form, data,
    family = ziGamma(link = "log"),
    ziformula = ~ 1,
    dispformula = ~ treatment
  ),
  hundhr = glmmTMB(form, data, 
    family = ziGamma(link = "log"),
