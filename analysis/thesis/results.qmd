...
species in LD compared to HA (p = 0.034), and interestingly, six times more other species
basal area in HD compared to HA (p = 0.014, @fig-mod-regen-ba).
...
@fig-mod-regen-ba-rw-to shows the same model as @fig-mod-regen-ba, but with an
emphasis treatment comparisons between redwood and tanoak. This shows that we
expect on average, five times greater basal area of redwood than tanoak in the
...
is expected to be 1.4 times that of tanoak (p = 0.56). The variability, in terms
of the 95% confidence intervals is much greater than that of tanoak's across
treatments.

```{r}
#| label: fig-mod-regen-ba-rw-to
#| fig-cap: >
#|   Basal area (m^2^ ha^-1^) modeled at the vegetation plot level for four
...
and macro-plot (@lst-mod-df-counts).
...
three-way interactions (@lst-mod-sprout-ht-inc).  

::: {#lst-mod-sprout-ht-inc}
```{r}
...
```{r}
#| label: fig-mod-sprout-ht-inc-treat
#| fig-cap: >
...
increment (@fig-mod-sprout-ht-inc-year).


```{r}
#| label: fig-mod-sprout-ht-inc-year
#| fig-cap: >
...
their interaction as predictors (@lst-mod-sprout-ht-yr-10).

::: {#lst-mod-sprout-ht-yr-10}
```{r}
...
followed the pattern GS > LD > HA > HD (@fig-mod-sprout-ht-yr-10).

```{r}
#| label: fig-mod-sprout-ht-yr-10
#| fig-cap: >
...
as a predictor (@tbl-mod-fuel-pre-pct).

```{r}
#| label: tbl-mod-fuel-pre-pct
...
very similar across treatments (p = 0.7 — p = 1). Point estimates varied by about
1, 3, and <20 Mg ha-1 for ten, hundred, and thousand-hour fuels, respectively
(@fig-mod-fuel-pre-pct).

```{r}
#| label: fig-mod-fuel-pre-pct
#| fig-width: 8
#| fig-cap: >
...
observations was used (@tbl-mod-fuel-post-pct).

```{r}
#| label: tbl-mod-fuel-post-pct
...
Post-pct resulted in greater stratification of treatments
(@fig-mod-fuel-post-pct). One-hour fuels were
