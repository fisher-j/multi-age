...
make_emm_res <- function(model, newdata, x_var, by_var, contrast) {
  newdata_sub <- rlang::enquo(newdata)
  by <- c(by_var, x_var)
  pred <- predictions(
    model,
    newdata = !!newdata_sub,
...
regen_all <- make_emm_res(
  mba,
  datagrid(spp = unique, treat = unique, site = unique),
  x_var = "treat",
  by_var = "spp",
...
pred <- predictions(
  mba,
  newdata = datagrid(spp = c("rw", "to"), treat = unique, site = unique),
  re.form = NULL,
  by = c("spp", "treat")
) |>
  group_by(treat) |>
...
comp <- comparisons(
  ...
  newdata = datagrid(spp = c("rw", "to"), treat = unique, site = unique),
  re.form = NULL,
  variables = list(spp = c("to", "rw")),
  by = c("treat", "contrast")
) |>
  group_by(treat) |>
  nest(.key = "comp")

res <- left_join(by = "treat", pred, comp) |>
  rowwise() |>
  mutate(emmobj = list(get_emmobj(pred, comp, "spp")))

regen_rw_to <- make_emm_res(
  mba,
  datagrid(spp = c("rw", "to"), treat = unique, site = unique),
  x_var = "spp",
  by_var = "treat",
...
emmeans(minc_sel, c("year"), by = "spp", component = "response") |>
  ...
  geom_dots(
    data = dinc,
    aes(year, ht_inc, ymin = NULL, ymax = NULL),
    alpha = 0.4
  ) +
  facet_wrap(~spp, labeller = plot_labels) +
