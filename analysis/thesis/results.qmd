...
According to predictions made from this model, there was not enough evidence to
support that treatments resulted in different basal area densities of
regeneration. This was true for species individually and when considered
together (data not shown). Although trends in means follow the expected pattern
of greater regeneration with less retention.

Redwood regeneration showed the strongest apparent treatment response, with the
greatest basal area observed in the GS treatment. LD and HD treatments were
intermediate, while HD tended to have the lowest redwood basal area.

Tanoak regeneration basal area was intermediate relative to redwood and
Douglas-fir. GS and LD treatments showed similar, comparatively higher
responses, whereas HA and HD tended to be lower.

Douglas-fir regeneration was low across all treatments, with little apparent
variation among them. GS showed the highest basal area, but overall differences
were minimal.

Other species (grand fir, madrone, and California wax-myrtle) occurred at low
densities across plots. Most plots contained few individuals, though one LD
macro plot had notably higher counts. Basal area for other species was low
across treatments overall, with HD tending to be highest and HA lowest, while GS
and LD were intermediate.
...
#| label: fig-regen-ba
#| fig-cap: >
#|   Basal area density (m^2^ ha^-1^) modeled at the vegetation plot level for
#|   four harvest treatments and four species classes (n = 16). Gray bars
#|   represent the asymtotic 95% confidence interval (α = 0.05), black dots
#|   indicate the mean, and blue arrows provide a means of assessing the
#|   statistical significance of pairwise differences among treatments within a
#|   species (panels). Arrows are drawn so that when two arrows just meet, the
#|   p-value for that difference is 0.05 and overlapping arrows indicate a
#|   p-values greater than 0.05. Note that Y-axes vary by species. While none of
#|   the differences were statistically supported, basal area was greatest in
#|   the GS treament, varied by species in the LD, and was lowest in the HA and
#|   HD. Additionally, redwood basal area was the greatest, tanoak was lower,
#|   and Douglas-fir and Other were much lower, but differences were smaller
#|   with HA and HD treatments.

fig_regen_ba <- regen_all |>
...
@fig-regen-ba-rw-to highlights the rapid decline in redwood basal area density
relative to tanoak with increasing overstory retention. It also illustrates the
greater uncertainty associated with redwood basal area density in the GS
treatment, as reflected by the wider confidence intervals compared to those of
tanoak.
...
#| fig-cap: >
#|   Basal area density (m^2^ ha^-1^) modeled at the vegetation plot level for
#|   four harvest treatments and two species classes (n = 16). Gray bars
#|   represent the 95% confidence interval (asymtotic, based on normal
#|   approximation), black dots---the mean, and non-overlapping blue arrows
#|   signify statistical significance (α = 0.05) of comparisons between species
#|   redwood and tanoak within a treatment. While none of the differences were
#|   statistically supported, this figure highlights the decreasing difference
#|   between redwood and tanoak with increasing overstory retention.

fig_regen_ba_rw_to <- regen_rw_to |>
  mutate(emm_df = list(emm_to_df_emm(emmobj))) |>
  select(treat, emm_df) |>
  unnest(emm_df) |>
  ggplot(emm_mapping(x = level)) +
...
The selected model for Douglas-fir counts did not indicate any statistically
significant differences between treatments. Generally, we expect about 2
seedlings per 4-meter-radius plot, or about 400 seedlings per hectare
(@fig-df-counts).
...
#|   approximation. Comparisons among treatment were not significant and
#|   centered around 400 seedlings per hectare.

df_counts <- marginaleffects_to_emmeans(
  model = mdf,
  newdata = list(treat = unique),
  x_var = "treat",
  by_var = NULL,
  contrast = list(treat = "pairwise")
)

fig_df_counts <- emm_to_df_emm(df_counts) |>
  mutate(across(where(is.double), \(x) x * per_ha["corner"])) |>
  ggplot(emm_mapping(x = level)) +
  draw_emm_emm() +
  labs(x = "Treatment", y = expression(Density ~ (stems %.% ha^-1)))
...
ht_inc_treat <- marginaleffects_to_emmeans(
  model = minc_sel,
  newdata = list(spp = unique, treat = unique, year = unique),
  x_var = "treat",
  by_var = "spp",
  contrast = list(treat = "pairwise")
)
...
#| fig-width: 5
#| fig-cap: >
#|   Estimated marginal means for the effect of harvest treatment (x-axis) on
#|   redwood and tanoak sprout height increment (y-axis), averaged over two
#|   growth periods, ten years after harvest. Gray bars represent asymtotic
#|   (normal approximation) confidence intervals and statistical significance (α
#|   = 0.05) is indicated by non-overlapping blue arrows for species (panels)
#|   between treatments. Height increment decreased with overstory retention
#|   were GS > LD > (HA & HD). The difference between GS, and HA and HD were
#|   statistically supported.

fig_sprout_ht_inc_treat <- ht_inc_treat |>
  mutate(emm_df = list(emm_to_df_emm(emmobj))) |>
  select(spp, emm_df) |>
  unnest(emm_df) |>
  ggplot(emm_mapping(x = level)) +
...
#|   data (small dots). Gray bars represent asymtotic (normal approximation)
#|   confidence intervals and statistical significance (α = 0.05) is indicated
#|   by non-overlapping blue arrows for a period (x-axis) within a growth period
#|   (panel). Both tanoak and redwood exhibited a slight decrease in height
#|   increment from the first to second periods.

fig_sprout_ht_inc_year <- ht_inc_year |>
  mutate(emm_df = list(emm_to_df_emm(emmobj))) |>
  select(spp, emm_df) |>
  unnest(emm_df) |>
  ggplot(emm_mapping(x = level)) +
...
ht_year_10 <- marginaleffects_to_emmeans(
  model = ht10_sel,
  newdata = list(spp = unique, treat = unique),
  x_var = "treat",
  by_var = "spp",
  contrast = list(treat = "pairwise")
)
...
#| fig-width: 5
#| fig-cap: >
#|   Predicted mean height (black dots) and 95% (asymtotic) confidence intervals
#|   (gray bars) for redwood and tanoak stump sprouts 10 years after harvest
#|   using four different harvest treatments. Non-overlapping blue arrows
#|   indicate statistically significant differences between treatments within a
#|   species (panels). Height was greatest in the GS treatment and lowest in the
#|   HA and HD treatments. Redwood was taller than tanoak.

fig_sprout_ht_yr_10 <- ht_year_10 |>
  mutate(emm_df = list(emm_to_df_emm(emmobj))) |>
  select(spp, emm_df) |>
  unnest(emm_df) |>
  ggplot(emm_mapping(x = level)) +
...
fuel_tmb$pre <- mutate(
  fuel_tmb$pre,
  newdata = list(tidyr::expand(data, treatment, site)),
...
#|   comparisons (blue arrows) of surface fuel loading across four treatments
#|   for six different fuel-class models. Non-overlapping blue arrows indicates
#|   statistical significance at the α = 0.05 level for treatments within a fuel
#|   class (panels). Differences between treatments for most classes were small
#|   and/or statistically unsupported, except for 1-hr and vegetation fuels for
#|   GS and HA treatment comparisons, where 1-hr was lower in the GS treatment
#|   and vegetation was higher.

fig_fuel_pre_pct <- fuel_tmb$pre |>
...
#| fig-width: 6.3
#| fig-cap: >
#|   Estimated marginal means (black dots), 95% confidence intervals (asymtotic,
#|   gray bars) and statistical comparisons (blue arrows) of fuel loading across
#|   four treatments for six different fuel-class models. Non-overlapping blue
#|   arrows indicates statistical significance at the α = 0.05 level within a
#|   fuel class (panels). Vegetation difference equals the transect level
#|   difference in vegetation load in the pre and post-pct conditions. This
#|   represents slash fuels recruited to the forest floor following the
#|   pre-commercial thinning. Loading was more differentianted following PCT,
#|   where 1-hr, 10-hr, 100-hr, and vegetation difference were all higher in the
#|   GS treatment.

fig_fuel_post_pct <- fuel_tmb$post |>
  mutate(class = fuel_class_labels[as.character(class)]) |>
  group_map(function(x, ...) {
    emm_to_df_emm(x$emmobj[[1]]) |>
      ggplot(emm_mapping(x = level)) +
...
#|   classes (panels), before and after PCT. Pre- and Post PCT models within a
#|   treatment are from similar, but not necessarily identical models.
#|   Ninety-five percent (asymtotic) confidence intervals are represented by
#|   lighter orange bars for pre-PCT and dark blue for post-PCT fuel loads. This
#|   figure shows how fuel loads inscrease after PCT for 1-, 10-, and to a
#|   lesser extent, 100-hr fuels, particularly for the GS treatment, while
#|   vegetation loading decreased across all treatments.

# pre and post together
fig_fuel_pct_comparison <- fuel_tmb$pre |>
...
save(
  fuel_class_labels,
  per_ha,
  format_model_formula,
  print_model_info,
  standard_model_col_names,
  regen_all,
  regen_grand_means,
  df_counts,
  ht_inc_treat,
  ...
  minc_sel,
  ht10_sel,
  file = "./results_appendix_deps.rda"
)
```
