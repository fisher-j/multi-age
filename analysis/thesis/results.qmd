...
`r regen_mean$rw$estimate` m^2^ ha^-1^, which was 
...
treatment, but this difference diminishes such that GS > LD > HA > HD. In the HD
treatment redwood and tanoak average basal area uncertainty across sites is very
similar. (@fig-regen-ba-rw-to).
...
#|   scaled to stems per hectare (4-meter radius plots). The asymptotic 95%
#|   confidence intervals are based on the normal approximation.
...
zeros, which was modeled with a logit link. For the 10-hr fuel model, the hurdle
...
After PCT, the response for all six fuel classes were modeled with a gamma
