...
![The vicinity, plot locations, and topography for my study plots. The small red
box in the inset delineates the area of the topographical map seen in the
background. Study plots appear in inset and background as black dos. JDSF =
Jackson Demonstration State Forest.](../figures/site_map_2.png){#fig-site-map}
...
2-hectare treatment units. 

The target post-harvest density was specified in terms of relative density,
defined as the proportion of the assumed total site carrying capacity based on
an upper stand density index (SDI) limit for redwood of 2,470 stems stems ha^-1^
[@reinekePerfectionStanddensityIndex1933]. The HA and HD treatments targeted a
residual overstory relative density of 20%, which was selected as an upper limit
to support the objective of maintaining conifer growth. In contrast, the LD
treatment targeted a residual overstory relative density of 13% to favor
establishment of a new cohort. These target densities were chosen to balance
stand-level volume production with individual tree growth, assuming a
subsequent, similar harvest entry after 20 years
...
![A diagram of the sampling design depicting a 0.2-hectare macro plot (large,
square, black outline), fuels transects (short, oragne, perpendicular lines),
sampling cylinders (small orange circles) where duff, litter, and live and dead
vegetation were measured, and vegetation sub-plots (medium, green circles),
where regeneration density and diversity were
quantified.](../figures/fuel_sampling_layout.png){#fig-plot-layout}

## Data collection

### Regeneration

Ten years after the initial harvest, four regeneration plots (4-m radius) were
established within each macro plot (@fig-plot-layout). All regenerating trees
(sprouts and seedlings) within the plot were recorded. Regenerating trees less
than 5 cm diameter at breast height (1.4 m) were tallied by 2.5 cm size classes
and the actual DBH of all other regenerating trees were recorded. This allowed
for an estimation of sprout and seedling density by species. 
...
tallest individual within the selected redwood and tanoak clumps at the time of
measurement with a height pole placed at the same location on the ground at each
measurement period. The observer eliminated parallax by observing from a
location up-slope, level with the height of the sprout. Sprout DBH for only
redwood was collected in year 10.
...
of sprouts. Although numerous metric have been used to asses shade tolerance
[@forresterResponseTreeRegeneration2014], basal area was selected because it
integrates both growth and survival of sprouts. This approach assumes that
growth and survival are positively correlated in forests that do not experience
prolonged harsh winters [@linSaplingGrowthSurvivorship2002]. 
...
$$
\log(\mu_i) = \beta^{\mu}_0 + 
\beta^{\mu}_{\text{trt}} \cdot \text{Trt} + 
\beta^{\mu}_{\text{spp}} \cdot \text{Spp} + 
\beta^{\mu}_{\text{trt:spp}}(\text{Spp} \times \text{Trt}) + 
\alpha_{j[i]}
$$
...
identity link. It included treatment, growth period, species, the interaction of
species and growth period, and the interaction of treatment and species  as
fixed effects. The treatment/species interaction was included not because the
interaction was significant, but because it represented a fundamental research
question. A random intercept was included for tree (multiple observations) and
macro-plot, and another random effect allowed the response to vary by species
differently for each macro plot. The dispersion parameter for the response was
modeled (with a log link) as a function of treatment, growth period, species and
...
$$
\mu_i = \beta_0^{\mu} + 
\beta_{\text{trt}}^{\mu} \cdot \text{Trt}_i + 
\beta_{\text{year}}^{\mu} \cdot \text{Year}_i + 
\beta_{\text{spp}}^{\mu} \cdot \text{Spp}_i +
\beta_{\text{spp:trt}}^{\mu}(\text{Spp}_i \times \text{Trt}_i) + 
\beta_{\text{spp:year}}^{\mu}(\text{Spp}_i \times \text{Year}_i) +
\alpha_{\text{tree}[i]} + 
\alpha_{\text{macro}[i],\text{spp}[i]}
$$
...
- $\alpha^{\mu}_{\text{macro}, \text{spp}}$: Random effect for species,
  co-varying within macro plot
- $\boldsymbol{\Sigma}_{\text{macro}}$: Unstructured variance-covariance matrix
  for species within macro plot


#### Height at year 10

Sprout heights at year 10 were modeled with a normal response and a log link.
The model with the lowest AIC included species and treatment, but no
interactions in the fixed effects. I included the interaction of species and
treatment because it was central to our question. I also included a model for
...
\beta_{\text{trt}}^{\mu} \cdot \text{Trt}_i + 
\beta_{\text{spp}}^{\mu} \cdot \text{Spp}_i +
\beta_{\text{spp:trt}}^{\mu}(\text{Spp} \times \text{Trt}) +
\alpha_{\text{spp}[i], \text{macro}[i]}
$$
...
- $\alpha_{\text{macro}, \text{spp}}$: Random effect for species, co-varying
  within macro plot
- $\boldsymbol{\Sigma}_{\text{macro}, \text{spp}}$: Unstructured
  variance-covariance matrix for species within macro plot
...
zero for that class and transect. 

Fuel load for 1- through 1,000-hr fuels were calculated using the method in
...
derived from a previous study where I used parameters averaged across all of the
measured plots [@glebockiFuelLoadingMoisture2015]. For 1,000-hr fuels, decay
classes 1-3 were considered "sound" and decay classes 4 and 5 were considered
"rotten" for the purpose of calculating load and these loads were combined. 

Duff and litter were measured at two locations along each transect and these
were averaged at the transect level. We did not measure the bulk density of duff
and litter fuels, but based on a suggestion in another study we used an average
combined bulk density (7.73 Mg ha^-1^) derived from average bulk density across
other studies in the redwood region
[@finneyFuelLoadingBulk1993;@stuartRedwoodFireEcology1985;@kittredgeComparisonForestFloors1940].
We opted to combine duff and litter components to simplify analysis and focus on
total forest floor fuel load.
...
[@gangiFIREMONDatabaseUser2006]. Live and dead vegetation components were combined to
calculate herbaceous and woody loading and herbaceous and woody fuel loads were
also combined and averaged at the transect level (two measurements per transect)
to determine total vegetation loading. Vegetation fuel classes were combined to
simplify analysis and because the herbaceous fuel load was generally very low.
...
- $\beta^{\cdot}_{\cdot}$: Fixed effects for intercepts and treatment levels for
  mean, hurdle, and dispersion components
...
After PCT, duff & litter was not modeled, vegetation difference was added. 
