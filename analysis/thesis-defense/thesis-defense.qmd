...
but why study multiaged management?
:::
...
- Growing interest in transformation in Redwood region
...
A diversity of silvicultural techniques supports ecological diversity which
...
So why do study multiaged management in redwood forests?
:::
...
::: {.notes}
We don't really understand how management approaches interact with fuels and
fire in these systems.

So I built in metrics to first-of-its-kind experiment to improve our
...
We'll start with implementation
:::
...
::: {.notes}
There were four treatments (harvest intensities) designed to reproduce a range
...
competition including light and growing space.

We installed one 1/5-ha plot within each 2-ha treatment block.
...
Next, we'll see how the treatments looked like on the ground.
:::
...
We'll start by defining the plot structure.
:::
...
::: {.notes}
Sprout heights were measured in the macro plot

Sprouts were selected to be well distributed across the macro plot. 

This was to give us a good estimate of individual tree growth response over
time.
...
In fact the sprouts had grown so much that a PCT was necessary, and that slash
would become downed woody fuel, so we wanted to characterize that fuel loading
...
Ok, that was a lot of trampling around that plot. Hopefully we didn't crush too
many sticks!

Lets review the data we collected.
:::
...
- All response variables were analyzed using mixed-effects models to account for
  nesting within sites, macroplots, corners, transects, and repeated measures
  where applicable.
...
- glmmTMB provides a consistent frame work for many distributions as well as sub-models
  for zero-inflation and dispersion.
- Candidate distributions and model structures were compared using AIC, and
  residual diagnostics were evaluated using R package, DHARMa.  
...
::: {.notes}
Fuel loads were calculated using methods of Brown (1974) using regionally
derived parameters.
...
tranformation to multiaged management.
:::
...
::: {.notes}
It's important to keep in mind here, that while we sampled many trees, 32
transects, 16 regeneration sub-plots per treatment. For the purpose of
...
statistical significance at the α=0.5 level. Β
...
The vegetation fuels may serve as ladder fuels under certain fire-weather
conditions.
...
This will likely require the use of active tending and prescribed fire to manage
these fuels. 
:::
